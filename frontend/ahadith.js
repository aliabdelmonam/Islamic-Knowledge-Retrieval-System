/* ==========================================================================
   Noor Al-Hidayah — الأحاديث Page Logic
   ========================================================================== */
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // ---- Favorites store helpers (shared format with app.js / duas.js / favorites.js) ----
    ...
    const emptyState = document.getElementById("hadithEmptyState");
    ...
    function applyFilters() {
        ...
        hadithCards.forEach((card) => {
            ...
            const textParts = [
                card.querySelector(".hadith-text")?.textContent || "",
                card.querySelector(".hadith-explanation")?.textContent || "",
                card.querySelector(".dua-category-tag")?.textContent || "",
                card.querySelector(".hadith-meta")?.textContent || ""
            ].join(" ");
            const cardText = normalizeArabic(textParts);
    ...
    // Hadith card actions: copy / favorite (event delegation)
    document.getElementById("hadithGrid").addEventListener("click", async (e) => {
        ...
        if (action === "copy") {
            const hadithText = card.querySelector(".hadith-text")?.textContent.trim() || "";
            const meta = card.querySelector(".hadith-meta")?.textContent.trim().replace(/\s+/g, " ") || "";
            const fullText = meta ? `${hadithText}\n\n${meta}` : hadithText;
            try {
        ...
        if (action === "favorite") {
            ...
            const meta = card.querySelector(".hadith-meta")?.textContent.trim().replace(/\s+/g, " ") || null;
            const id = "hadith_" + simpleHash(tag + "|" + hadithText);
            ...
            if (exists) {
                ...
                favorites.push({ id, type: "hadith", title: tag, text: hadithText, meta, date: new Date().toISOString() });
